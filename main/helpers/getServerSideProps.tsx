...
export default async function myGetServerSideProps(context: GetServerSidePropsContext) {
  ...
  if (token) {
    ...
    if (hasToken) {
      ...
      const response = await fetch(process.env.API_URL + `/api/user?method=getOne`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          _id: id,
